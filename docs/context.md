...
3. 匹配即制度（铁律五）：ClearingHouse 不再包含 match_market / match_plan。分配算法本身是用户策略（MatchingPolicy），属于可插拔层。结算器只负责不可变的物理规则。
...
正向范式二：匹配即制度。
计划经济和市场经济的本质区别不在于"如何算产量"，而在于"Bid/Ask 如何分配"。因此分配算法本身必须是用户策略而非内核。这使得实验者可以对比同一组主体在不同匹配规则下的宏观涌现。
...
已解决 — 匹配逻辑架构：已确立铁律五（匹配即制度），ClearingHouse 不再包含分配算法。

阶段八（本次对话）：供需池架构与术语标准化
已解决 — 供需池重构：WorldState 新增 supply_pool 和 demand_pool（持久订单池），替代"每 Tick 新建→立即分配→丢弃"的挥手即忘模式。订单现在在池中持续挂存，直到被分配、撤销或过期。
...
无 ProductionFn 独立槽位（生产逻辑内化于 FirmStrategy），无 LaborOrder 实体（劳动力为 good_type='labor' 的 Good），无 match_labor 方法（由统一 match 分配）。
